...
func PingMessageFromSlice(slice []byte, compatible bool) (*PingMessage, error) {
	...
	if uint32(sliceLen) < headerSize {
		errMsg := strings.Join([]string{"HeaderIsBroken", "PingMessage", strconv.Itoa(int(sliceLen)), "<", strconv.Itoa(int(headerSize))}, " ")
		return nil, errors.New(errMsg)
	}
...
func PingFromSlice(slice []byte, compatible bool) (*Ping, error) {
	...
	headerSize := HeaderSizeUint * (uint32(fieldCount) + 1)
	if uint32(sliceLen) < headerSize {
		errMsg := strings.Join([]string{"HeaderIsBroken", "Ping", strconv.Itoa(int(sliceLen)), "<", strconv.Itoa(int(headerSize))}, " ")
		return nil, errors.New(errMsg)
	}
...
func PongFromSlice(slice []byte, compatible bool) (*Pong, error) {
	...
	headerSize := HeaderSizeUint * (uint32(fieldCount) + 1)
	if uint32(sliceLen) < headerSize {
		errMsg := strings.Join([]string{"HeaderIsBroken", "Pong", strconv.Itoa(int(sliceLen)), "<", strconv.Itoa(int(headerSize))}, " ")
		return nil, errors.New(errMsg)
	}
