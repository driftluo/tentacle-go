...
func StringVecFromSlice(slice []byte, compatible bool) (*StringVec, error) {
	...
	if uint32(sliceLen) < headerSize {
		errMsg := strings.Join([]string{"HeaderIsBroken", "StringVec", strconv.Itoa(int(sliceLen)), "<", strconv.Itoa(int(headerSize))}, " ")
		return nil, errors.New(errMsg)
	}
...
func ProtocolInfoMolFromSlice(slice []byte, compatible bool) (*ProtocolInfoMol, error) {
	...
	if uint32(sliceLen) < headerSize {
		errMsg := strings.Join([]string{"HeaderIsBroken", "ProtocolInfoMol", strconv.Itoa(int(sliceLen)), "<", strconv.Itoa(int(headerSize))}, " ")
		return nil, errors.New(errMsg)
	}
